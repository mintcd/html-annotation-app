    ...
    for (const op of ops) {
      try {
        ...
        if (payload && payload.action === 'insert' && payload.data) {
          await localDb.putRow(entity as any, payload.data);
          applied++;
          continue;
        }
        if (payload && payload.action === 'update' && (payload.changes || payload.id)) {
          const id = payload.id;
          if (id !== undefined) {
            const row = await localDb.getRow(entity as any, id);
            const newRow = Object.assign({}, row || {}, payload.changes || {});
            await localDb.putRow(entity as any, newRow as any);
            applied++;
            continue;
          }
        }
        if (payload && payload.action === 'delete' && payload.id !== undefined) {
          await localDb.deleteRow(entity as any, payload.id);
          applied++;
          continue;
        }
        ...
        if (opType === 'insert' && payload) {
          await localDb.putRow(entity as any, payload);
          applied++;
          continue;
        }
